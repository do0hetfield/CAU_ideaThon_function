...
filepath = '/Users/choijunhyeok/Desktop/ideathon_FE/src/GuidePage.tsx'

with open(filepath, 'r', encoding='utf-8', errors='ignore') as f:
    content = f.read()

# Define the new clause data and contract body
new_content = """// ─── 조항 데이터 ───────────────────────────────────────────────────────
...
# Let's find the boundaries using str.find
start_idx = content.find('// ─── 조항 데이터')
if start_idx == -1:
    # try just 'const CLAUSE_DATA'
    start_idx = content.find('const CLAUSE_DATA')

end_idx = content.find('// ─── 수정된 조항 반영 미리보기 컴포넌트')

if start_idx != -1 and end_idx != -1:
    before = content[:start_idx]
    after = content[end_idx:]
    
    final_content = before + new_content + after
